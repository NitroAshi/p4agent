...
def test_parse_valid_command() -> None:
    ...
    assert result.command.payload_json == '{"target_file":"./aaa.txt"}'


def test_parse_rejects_missing_prefix() -> None:
    result = parse_agent_pr_comment(
        'append_hello_agent_comment "{\\"target_file\\":\\"./aaa.txt\\"}"',
        supported_task_ids={"append_hello_agent_comment"},
    )
...
def test_parse_rejects_unsupported_task() -> None:
    result = parse_agent_pr_comment(
        '/agent-pr arbitrary_task "{\\"target_file\\":\\"./aaa.txt\\"}"',
        supported_task_ids={"append_hello_agent_comment"},
    )
...
def test_parse_rejects_wrong_arity() -> None:
    result = parse_agent_pr_comment(
        "/agent-pr append_hello_agent_comment",
        supported_task_ids={"append_hello_agent_comment"},
    )
...
def test_parse_rejects_non_object_payload() -> None:
    result = parse_agent_pr_comment(
        "/agent-pr append_hello_agent_comment '[1,2,3]'",
        supported_task_ids={"append_hello_agent_comment"},
    )
...
def test_write_output_github_format(tmp_path: Path) -> None:
    ...
    assert output_file.read_text(encoding="utf-8") == (
        ...
        "payload_json={\"target_file\":\"./aaa.txt\"}\n"
        "error_message=\n"
    )
